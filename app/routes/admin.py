...
from app.schemas.admin import (
    ...
    BulkUpdateProductVerificationRequest,
    BulkUpdateProductVerificationResponse,
...
async def delete_admin_product(
    ...
    Permanently deletes a product and all associated data including:
    - Product follows
    - Product reports
    - Spotlights and spotlight history
    - Orders (if any)
    ...
    safe_delete(
        lambda: db.query(ProductFollow).filter(ProductFollow.product_id == product_id_int).delete(synchronize_session=False),
        "product follows"
    )
    
    # Delete product reports
    safe_delete(
        lambda: db.query(ProductReport).filter(ProductReport.product_id == product_id_int).delete(synchronize_session=False),
        "product reports"
    )
    
    # Delete spotlight history for this product
    safe_delete(
        lambda: db.query(SpotlightHistory).filter(SpotlightHistory.product_id == product_id_int).delete(synchronize_session=False),
        "spotlight history"
    )
    
    # Delete spotlights for this product
    safe_delete(
        lambda: db.query(Spotlight).filter(Spotlight.product_id == product_id_int).delete(synchronize_session=False),
        "spotlights"
    )
    ...
    try:
        ...
        safe_delete(
            lambda: db.query(Order).filter(Order.product_id == product_id_int).delete(synchronize_session=False),
    ...
    # Finally, delete the product (this is the critical operation)
    try:
        db.delete(product)
        db.commit()
    except Exception as e:
        db.rollback()
        error_msg = str(e)
        logger.error(f"Failed to delete product {product_id_int}: {error_msg}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Cannot perform this operation due to related records",
            headers={"X-Error-Type": "Database constraint violation"}
        )
...
@router.delete("/users/bulk", response_model=BulkDeleteUsersResponse)
async def bulk_delete_users(
    ...
    """
    Bulk delete multiple users (Admin or Moderator with can_manage_users permission).
    
    Permanently deletes multiple user accounts and all associated data.
    All operations are performed in a transaction - if any deletion fails,
    all changes are rolled back.
    
    Args:
        request_data: Request containing list of user IDs to delete
        current_user_id: Current authenticated user ID
        db: Database session
        
    Returns:
        BulkDeleteUsersResponse: Response with deleted count and IDs
    ...
    for user_id in user_ids:
        ...
        elif str(user.id) == current_user_id:
            # Prevent admin from deleting themselves
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Cannot delete yourself (user ID: {user_id})"
            )
    ...
    try:
        for user_id in user_ids:
            ...
            for product in user_products:
                ...
                safe_delete(
                    lambda: db.query(ProductFollow).filter(ProductFollow.product_id == product_id_int).delete(synchronize_session=False),
                    f"product follows for product {product_id_int}"
                )
                safe_delete(
                    lambda: db.query(ProductReport).filter(ProductReport.product_id == product_id_int).delete(synchronize_session=False),
                    f"product reports for product {product_id_int}"
                )
                safe_delete(
                    lambda: db.query(SpotlightHistory).filter(SpotlightHistory.product_id == product_id_int).delete(synchronize_session=False),
                    f"spotlight history for product {product_id_int}"
                )
                safe_delete(
                    lambda: db.query(Spotlight).filter(Spotlight.product_id == product_id_int).delete(synchronize_session=False),
                    f"spotlights for product {product_id_int}"
                )
                ...
                try:
                    ...
                    db.rollback()
                    logger.warning(f"Could not delete product {product_id_int}: {e}")
            
            # Delete user follows (as follower)
            safe_delete(
                lambda: db.query(SellerFollow).filter(SellerFollow.follower_id == user_id).delete(synchronize_session=False),
                f"user follows (follower) for user {user_id}"
            )
            
            # Delete user follows (as followed)
            safe_delete(
                lambda: db.query(SellerFollow).filter(SellerFollow.followed_user_id == user_id).delete(synchronize_session=False),
                f"user follows (followed) for user {user_id}"
            )
            
            # Delete product follows by this user
            safe_delete(
                lambda: db.query(ProductFollow).filter(ProductFollow.user_id == user_id).delete(synchronize_session=False),
                f"product follows by user {user_id}"
            )
...
@router.put("/users/bulk/status", response_model=BulkUpdateUserStatusResponse)
async def bulk_update_user_status(
    ...
    """
    Bulk update user active status (Admin or Moderator with can_manage_users permission).
    
    Updates the is_active status for multiple users at once.
    
    Args:
        request_data: Request containing list of user IDs and new status
        current_user_id: Current authenticated user ID
        db: Database session
        
    Returns:
        BulkUpdateUserStatusResponse: Response with updated count and IDs
    ...
    try:
        ...
        return BulkUpdateUserStatusResponse(
...
async def delete_admin_user(
    ...
    def safe_delete(query_func, error_message):
        ...
        try:
            result = query_func()
            if result is not None:  # Some queries might return None
                db.commit()  # Commit the deletion
                if result > 0:
                    logger.info(f"Deleted {result} record(s) from {error_message}")
            else:
                db.commit()  # Still commit even if result is None
        except Exception as e:
            # Rollback the transaction to reset the failed state
            db.rollback()
            error_str = str(e)
    ...
    try:
        from app.models.order import Order
        safe_delete(
            lambda: db.query(Order).filter(Order.product_id.in_(user_product_id_list)).delete(synchronize_session=False) if user_product_id_list else None,
    ...
    if user_product_id_list:
        safe_delete(
            lambda: db.query(ProductFollow).filter(ProductFollow.product_id.in_(user_product_id_list)).delete(synchronize_session=False),
    ...
    # Delete product follows
    safe_delete(
        lambda: db.query(ProductFollow).filter(ProductFollow.user_id == user_id_int).delete(synchronize_session=False),
        "product follows"
    )
    
    # Delete moderator permissions
    safe_delete(
        lambda: db.query(ModeratorPermission).filter(ModeratorPermission.user_id == user_id_int).delete(synchronize_session=False),
    ...
    try:
        ...
        # Delete spotlights
        safe_delete(
            lambda: db.query(Spotlight).filter(Spotlight.applied_by == user_id_int).delete(synchronize_session=False),
        ...
        if user_product_id_list:
            safe_delete(
                lambda: db.query(Spotlight).filter(Spotlight.product_id.in_(user_product_id_list)).delete(synchronize_session=False),
    ...
    # Delete product reports by this user
    safe_delete(
        lambda: db.query(ProductReport).filter(ProductReport.user_id == user_id_int).delete(synchronize_session=False),
    ...
    if user_product_id_list:
        safe_delete(
            lambda: db.query(ProductReport).filter(ProductReport.product_id.in_(user_product_id_list)).delete(synchronize_session=False),
            "product reports about user's products"
        )
    
    # Delete email verifications
    safe_delete(
        lambda: db.query(EmailVerification).filter(EmailVerification.email == user_email).delete(synchronize_session=False),
        "email verifications"
    )
    
    # Delete password reset tokens
    safe_delete(
        lambda: db.query(PasswordResetToken).filter(PasswordResetToken.email == user_email).delete(synchronize_session=False),
        "password reset tokens"
    )
    
    # Delete registration data
    safe_delete(
        lambda: db.query(RegistrationData).filter(RegistrationData.email == user_email).delete(synchronize_session=False),
        "registration data"
    )
    
    # Delete audit logs performed by this user
    # Note: We delete audit logs to maintain referential integrity
    # This removes the audit trail for actions performed by this user
    from app.models.audit_log import AuditLog
    safe_delete(
        lambda: db.query(AuditLog).filter(AuditLog.performed_by_id == user_id_int).delete(synchronize_session=False),
...
# ============================================================================
# BULK ACTION ENDPOINTS - Must come before parameterized routes
# ============================================================================

@router.post("/reports/bulk/approve", response_model=BulkApproveReportsResponse)
async def bulk_approve_reports(
    request_data: BulkApproveReportsRequest,
    current_user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db)
):
    """
    Bulk approve multiple reports (Admin or Moderator with can_manage_flagged_content permission).
    
    Approves multiple reports and deactivates the associated products.
    
    Args:
        request_data: Request containing list of report IDs to approve
        current_user_id: Current authenticated user ID
        db: Database session
        
    Returns:
        BulkApproveReportsResponse: Response with approved count and IDs
    ...
    # Get approved status
    approved_status = db.query(ReportStatus).filter(ReportStatus.status == "approved").first()
    ...
    if not_found_ids:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"One or more reports not found: {not_found_ids}"
        )
    ...
    try:
        ...
        return BulkApproveReportsResponse(
...
@router.post("/reports/bulk/reject", response_model=BulkRejectReportsResponse)
async def bulk_reject_reports(
    request_data: BulkRejectReportsRequest,
    current_user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db)
):
    """
    Bulk reject multiple reports (Admin or Moderator with can_manage_flagged_content permission).
    
    Rejects multiple reports by deleting them from the database.
    
    Args:
        request_data: Request containing list of report IDs to reject
        current_user_id: Current authenticated user ID
        db: Database session
        
    Returns:
        BulkRejectReportsResponse: Response with rejected count and IDs
    ...
    try:
        ...
        return BulkRejectReportsResponse(
